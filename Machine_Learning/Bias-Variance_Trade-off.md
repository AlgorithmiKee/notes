...
* Model I: fit a line: underfitting
  * poor performance on test data even trained on inf many training samples.
...
* Model II: fit a polynomial of degree 5: overfitting
  ...
  * but: good performance on test data if trained on inf many training samples.
...
> $$
> \begin{align}
> \mathbf{y}_i &= f^\star(\mathbf{x}_i) + \mathbf{e}_i \\
> \mathbf{x}_i &\sim p_X \\
> \mathbf{e}_i &\sim \mathcal{N}(0, \sigma^2) \\
> \end{align}
> $$
>
> * **Training**: To estimate the the ground truth $f^\star$, we use some ML algorithm $\mathcal{A}: D \mapsto \hat{f}_D$ to obtain a trained model $\hat{f}_D$.
>
...
Throughout the derivation, we use the square loss as the performance metric
...
TODO. Why minimizing training error is not enough?

## Decomposition of Test Error

Throughout the derivation, we use the square loss as performance metric
$$
\begin{align}
\ell(\hat{f}_D(x), y) \triangleq (\hat{f}_D(x) - y)^2
\end{align}
...
* In signal reconstrution, there is often no randomness in $x$. e.g. In polynomial fitting, the training samples $\{x_t\}_{t=1}^T$ are fixed: $x_t = [ 1, t, t^2, \dots, t^d]^\top$. In such application, the joint distribution simplifies to $p_{XY}(x,y) = p_\mathbf{e}(y - f^\star(x))$. More details: $\to $ *Fixed-Design Linear Regression*.
...
> $$
> \begin{align}
> \mathbb{E}_{XY} \left[ \ell(\hat{f}_D(x), y) \right]
...
* The RHS shows that the test error of a specific model $\hat{f}_D$ can be decomposed into
...
For fixed-design regression problems, the expectation $\mathbb{E}_{XY}$ is simplified to $\mathbb{E}_{\mathbf{e}}$. Hence, the formulae simplifies to
...
But $\mathbb{E}_{XY} \big[ \ell(\hat{f}_D(x), y) \big]$ only calulates the expected test error of a **specific model** $\hat{f}_D$. Suppose that we run our ML algorithm $\mathcal A$ on different training data sets. As mentioned before, we will get a different model on different training data set. If we pointwise average $\hat{f}_D$ over inf many training sets, we get the **averaged model**
...
* $\bar{f}$ is a hypothetical model for theoretical study. It can't be obtained in reality since we do not have inf many training sets.
* Later on, we will define
  * the bias as the difference between the average model $\bar{f}$ ground truth $f^\star$ and variance as the spread
  * the variance as the spread of $\hat{f}_D$ w.r.t. $\bar{f}$. i.e. How sensitive is the model $\hat{f}_D$ to the training set $D$​.

Hence, we would like to incorperate two sources of randomness:

1. The random data set $D \sim p_{XY}^n$, on which we obtain the trained model $\hat{f}_D$ throutgh ML algorithm​​.
2. The random test sample $(x,y) \sim p_{XY}$, on which we calcluate the expected test error.

and get  
> **Expected Test Error** of the ML algorithm $\mathcal{A}$:
> $$
> \begin{align}
> \mathbb{E}_{XY, D} \left[ \ell(\hat{f}_D(x), y) \right]
...
* The RHS is more interesting as the expected test error comprises three parts
  ...
  * **Bias**: This reflects asympotic property of our ML algorithm. We hope that the averaged model converges to the ground truth as we increase the number of training sets.
...
If the model complexity is too low, the bias remains high no matter how many training sets we have. The resulting model suffers from underfitting because it fails to capture complex relation between feature and label.

If the model complexity is too high, the variance will be large since TODO
...
Using the result from previous section
$$
\begin{align*}
&\mathbb{E}_D \left[
\mathbb{E}_{X} \left[\left(\hat{f}_D(x) - f^\star(x) \right)^2\right] + \sigma^2
...
&= \mathbb{E}_{X} \left[ \mathbb{E}_{D} \left[\big(\hat{f}_D(x) - \bar{f}(x)\big) \right] \cdot \big( \bar{f}(x) -f^\star(x) \big)\right] \\
&= \mathbb{E}_{X} \left[ 0 \cdot \big( \bar{f}(x) -f^\star(x) \big)\right] \\
&= 0
\end{align*}
$$
