...
Even though both $p(z)$ and $p(\mathbf{x} \mid z)$ are simple distributions (Gaussian), the marginal distribution $p(\mathbf{x})$ can be more complex. At $\sigma = 0.1$, the marginal distribution $p(\mathbf{x})$ appears to be heart-shaped. Namely, we can use latent variable model to form a complex distribution from simple ones. The core idea why it works is that $\boldsymbol{\mu}(z)$ can be highly non-linear. Extending this idea to higher dimension leads to decoder network of VAEs.
...
$$
\begin{align*}
\pi_1,\dots,\pi_K &\ge 0 \quad\text{s.t. } \sum_{k=1}^K \pi_k = 1 \\
\boldsymbol{\mu}_1, \dots, \boldsymbol{\mu}_K &\in \mathbb R^d, \quad
\boldsymbol{\Sigma}_1, \dots, \boldsymbol{\Sigma}_K \in \mathbb R^{d \times d}

\end{align*}
$$

The data is generated by two steps:

1. sampling $z$ from the categorical distribution, specified by $\pi_1,\dots,\pi_K$.
1. sampling $\mathbf{x}$ from the Gaussian distribution $\mathcal N(\boldsymbol{\mu}_z, \boldsymbol{\Sigma}_z)$.
...
* The conditional covariance matrix is $\mathbb V[\mathbf{x} \mid \mathbf{z}] = \sigma^2\mathbf{I}$ where $\sigma$ is a hyper parameter. (**Question**: is $\sigma$ a hyperparameter or a function of $z$)

The resulting latent variable model is
...
&\sim \mathcal N(\boldsymbol{\mu}_{\theta}(z), \sigma^2\mathbf{I})
\\
p(\mathbf{x}, \mathbf{z})
&= \mathcal N(z ; \mathbf{0},\mathbf{I}) \cdot
   \mathcal N(\mathbf{x} ; \boldsymbol{\mu}_{\theta}(z), \sigma^2\mathbf{I})
\end{align}
$$
