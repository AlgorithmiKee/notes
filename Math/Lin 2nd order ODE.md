...
- the functions $p(t)$ and $q(t)$ to represent the properties of the system. If they are constants, we say the system is **time-invariant**. Otherswise, we say the system is **time-varying**.
- the function $f(t)$ to represent an external input acting on the system. If $f(t)$ is zero, we say the system is **unforced**. Otherwise, we say the system is **forced**.
...
## Structure of the Solutions to Homogeneous ODEs
...
*Proof*: Let $\mathcal{S}$ be the set of all solutions to the homogeneous equation. It is easy to verify that $\mathcal{S}$ is a vector space. To show that $\dim(\mathcal{S}) = 2$, we use the existence and uniqueness theorem for ODEs:

Given any initial conditions $y(t_0) = y_0$ and $y'(t_0) = y_1$, there exists a unique solution to the homogeneous equation.
...
## Structure of the Solutions to Inhomogeneous ODEs

> **Theorem**: The general solution to the inhomogeneous equation can be expressed as the sum of the general solution to the corresponding homogeneous equation and a particular solution to the inhomogeneous equation:
> 
> $$
> y(t) = y_h(t) + y_p(t)
> $$
...
### Additivity of Particular Solutions
...
Using Euler's formula, the solution space can be expressed using another basis:
...
### Homogeneous ODE with Varying Coefficients

In the case of varying coefficients, the methods for finding solutions are generally more complex and may not yield closed-form solutions. If we can guess a solution, we can often use the method of reduction of order to find a second linearly independent solution.
...
3. **Laplacian Transform**: This method is particularly useful for solving ODEs with constant coefficients and can be used to find a particular solution by transforming the ODE into an algebraic equation in the Laplace domain.

Here, we will focus on the method of undetermined coefficients and variation of parameters.

### Inhomogeneous ODEs with Constant Coefficients
